...
contract CliptoExchange is CliptoExchangeStorage, Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getBountyRequest(bytes32 bountyHash) external view returns (Request memory) {
        return bountyRequests[bountyHash];
    }
    ...
    function newBountyRequest(
        ...
        _checkAllowance(_erc20, msg.sender, _amount);
        _pay(msg.sender, address(this), _erc20, _amount);
        bountyRequests[bountyHash] = Request(msg.sender, _nftReceiver, _erc20, _amount, false);
        emit NewBountyRequest(bountyHash, _jsonData);
    }

    function nativeBountyNewRequest(
         bytes32 bountyHash,
        address _nftReceiver,
        string calldata _jsonData
    ) external payable whenNotPaused {
         require(msg.value > 0 , "error: invalid amount");
        bountyRequests[bountyHash] = Request(msg.sender, _nftReceiver, address(0), msg.value, false);
        emit NewBountyRequest(bountyHash, _jsonData);
    }

    function deliverBountyRequest(
        string calldata hashWord, 
    ...
    function refundBountyRequest(string calldata hashWord) external nonReentrant whenNotPaused {
       bytes32 bountyHash =  keccak256(abi.encode(hashWord));
        ...
        emit RefundedBountyRequest(bountyHash);
    }
